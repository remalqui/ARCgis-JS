...
const browser = require('./browser-d9861345.js');
...
function isChrome109OrAbove() {
  const uaData = browser.getUserAgentData();
  if (uaData?.brands) {
...
async function patchFloatingUiForNonChromiumBrowsers() {
  if (config.floatingUINonChromiumPositioningFix &&
    ...
    (/firefox|safari/i.test(browser.getUserAgentString()) || isChrome109OrAbove())) {
    const { offsetParent } = await Promise.resolve().then(function () { return require('./composed-offset-position.esm-2b28a9f7.js'); });
